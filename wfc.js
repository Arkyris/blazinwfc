...
export default function WFC(definition) {
    ...
    let coords;
    let cell;
    const numLayers = definition.options.numLayers || 1;
    let layers = null;
    const collapseType = definition.options.collapseType || 'normal';

    if (numLayers > 1 && definition.layers) {
        layers = definition.layers;
    }

    // Fill the tiles array with the tiles from the definition
    for (let i = 0; i < definition.tiles.length; i++) {
        tiles.push(createTile(definition.tiles[i].edges, i, definition.tiles[i].type, definition.tiles[i].exceptions, definition.tiles[i].weight || definition.options.baseWeight, definition.tiles[i].layer || 0));
    }
    ...
    /**
     * @description: Selects the correct collapse function based on the collapseType defined in the options
     * @param {Number} width - The width of the map
     * @returns The final map generated by the algorithm
     */
    function collapse(width) {
        if (collapseType === 'normal') {
            return collapseNormal(width);
        } else if (collapseType === 'layers') {
            return collapseLayers(width);
        }
    }

    /**
     * @description: Collapse a new grid using the WFC algorithm and return the final map
     * @param {Number} newWidth - The width of the map
     * @returns {Number[][]} this.finalMap
     */
    function collapseNormal(newWidth) {
        resetState(newWidth);
    ...
    /**
     * @description: Collapse a new grid using the WFC algorithm and return the final map with layers
     * @param {Number} width - The width of the map
     * @returns {Number[][][]} finalMap [layer][x][y]
     */
    function collapseLayers(newWidth) {
        resetState(newWidth);
        while (collapsedCells < totalCells) {
            if (collapsedCells === 0) {
                cell = getRandomCell();
                ...
                collapseCell(cell);
                changedCells.push({ x: cell.x, y: cell.y });
                if (layers[tiles[cell.index].layer].fillTile !== cell.index) {
                    finalMap[tiles[cell.index].layer][cell.x][cell.y] = cell.index;
                }
                updateNeighbors(cell);
            } else {
                coords = getNextCell();
    ...
    /**
     * @description: Rollback the algorithm to the saved state for layer maps
     */
    function rollbackLayers() {
        for (let i = 0; i < changedCells.length; i++) {
            grid[changedCells[i].x][changedCells[i].y] = JSON.parse(JSON.stringify(previousStates.grid[changedCells[i].x][changedCells[i].y]));
        }
